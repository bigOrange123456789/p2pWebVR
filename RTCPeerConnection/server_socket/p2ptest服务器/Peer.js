...
class _Peer{
    ...
    connect(id){
        if(!this.idle()){
            alert('The previous connection has not been completed')
        }else if(this.peers[id]){
            alert('This connection has been established:'+id)
        }else{
            this.otherId=id
    ...
    onDataChannelCreated(channel) {
        ...
        channel.onclose=()=>{//当接收到close事件时候的事件处理器。当底层链路被关闭的时候会触发该事件。
            ...
            setTimeout(()=>{//对于关闭的通道应该重新连接
                //scope.connect(channel.peerId)
                //scope.addUser(channel.peerId)
