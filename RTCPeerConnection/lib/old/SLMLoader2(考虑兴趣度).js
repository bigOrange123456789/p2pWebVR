...
var SLMSceneBuilder = function(sceneMgr)
{
	...
	function InstanceNode(gltfScene, iMatrix, structDesc, geoInfo, propInfo)
	{
		...
		m1.visible = false;
		/*
		var rotation=instanceRoot.rotation;
		...
		*/
		...
		var projectName="JinYue"
		/*var countInit=100;
		for(var meshIndex0=1;meshIndex0<countInit;meshIndex0++)
			loadSubZip("assets/models/"+projectName+"/output"+meshIndex0+".zip", (gltf)=>{
				var group=gltf.scene.children[0];
				var myMeshEx1=group.children[0];
				var name=myMeshEx1.name;
				//console.log(name)
				instanceRoot.add(myMeshEx1)
				var myMeshEx2=processMesh(
					myMeshEx1,
					structDescription[meshIndex0],
		...
		test(countInit)
		var myIndexTag=countInit;
		var si=setInterval(()=>{
			console.log(myIndexTag)
			for(var i=0;i<500;i++)
			if(myIndexTag<6000){//6500
			//if(myIndexTag<structDescription.length){
				test(myIndexTag)
				myIndexTag=myIndexTag+1
			}else {
				clearInterval(si)
				console.log("加载时间为："+
					Math.round((performance.now()-window.time0)/1000)+"秒"
				)
				return;
			}
		},0)*/
		...
		function test(meshIndex) {
			loadSubZip("assets/models/"+projectName+"/output"+meshIndex+".zip", (gltf)=>{
				//if(meshIndex+1<structDescription.length) test(meshIndex+1)
				//if(meshIndex+1<5000)test(meshIndex+1)
				var group=gltf.scene.children[0];
				var myMeshEx1=group.children[0];
				//var name=myMeshEx1.name;
				//console.log(name)
