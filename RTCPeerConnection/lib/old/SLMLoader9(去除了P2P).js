	...
	function InstanceNode(gltfScene, iMatrix, structDesc, geoInfo, propInfo)
	{
		...
		function loading(meshIndex,linkNext,high_quality) {
			...
			loadSubZip("assets/models/"+projectName+"/output"+meshIndex+".zip", (gltf)=>{
			    ...
			    //进行双面渲染
				//for(i=0;i<myMeshEx2.material.length;i++)
				//	myMeshEx2.material[i].side=2


				//console.log(myMeshEx2.material)
				//用于提取外壳
				/*newMaterial=[]
				for(i=0;i<myMeshEx2.material.length;i++){
					material=new MeshBasicMaterial({color:meshIndex})
					material.color.convertSRGBToLinear();
					newMaterial.push(material)
				}
				myMeshEx2.material1=myMeshEx2.material
				myMeshEx2.material2=newMaterial
				//myMeshEx2.material=
				*/
