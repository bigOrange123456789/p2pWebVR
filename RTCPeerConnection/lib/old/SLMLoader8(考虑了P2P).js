...
var SLMLoader = function(options)
{
	...
	{
		/*var scenes="assets\\models\\"
			+window.location.href.split("scene=")[1].split("&")[0]
			+".zip"*/
...
var SLMSceneBuilder = function(sceneMgr)
{
	...
	function InstanceNode(gltfScene, iMatrix, structDesc, geoInfo, propInfo)
	{
		...
		var branching=2;//loading(100,false)
		console.log("分支数目为：",branching)//loading(1,true)
		function loading(meshIndex,linkNext) {
			if(window.location.href.split("onlyP2P=true").length>1)
				return
		...
		window.p2p.receive=(idData,array)=>{
			...
			new GLTFLoaderEx(new LoadingManager()).parse(
				array, "./",(gltf)=>{
					console.log("用P2P解析成功！",idData)
					addModel(gltf,idData,false)
			})
		}
		function addModel(gltf,meshIndex,linkNext) {
			...
			console.log(myMeshEx1)
			instanceRoot.add(myMeshEx1)
			var myMeshEx2=processMesh(
				myMeshEx1,
				structDescription[meshIndex],//structDescription[parseInt(name)],
				slmSceneMeta,
			...
			var countAll=6500//structDescription.length
			if(linkNext)
...
function loadSubZip (back,meshIndex,linkNext) {//assets\models\SAM_Review_1\output1.zip
	...
	new Promise( function( resolve, reject ) {
		...
		myGLTFLoaderEx.load(configJson.fileUrl, (gltf) => {//解析模型资源
			//console.log("this.myArray",myGLTFLoaderEx.myArray)
			window.p2p.send({
				type:"data",
				idData:meshIndex,
			...
			myGLTFLoaderEx.parse( myGLTFLoaderEx.myArray, "./",(gltf)=>{
				if(back)back(gltf,meshIndex,linkNext)
			})
		});
	} );
}
