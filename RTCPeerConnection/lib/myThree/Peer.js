window.peers={}
window.receive=(data)=>{console.log('receive',data)}
    ...
    function PeerInitiator(config) {
        ...
        function setChannelEvents(channel) {
            ...
            channel.onopen = function() {
                window.peers[Math.random()]=channel
...
class Peer{//对 _Peer 对象进行封装
    constructor(opt) {
        ...
        rtConnection.onopen = function (conn) {
            //scope.peers[conn.userid]=conn.channel
        };
        ...
        this.send=(data,target)=>{
            ...
            if(typeof target==="undefined")
                for(var i in window.peers){
                    if(window.peers[i].readyState==='open'){
                        if(window.peers[i].bufferedAmount+data.length>43690*300){
                            console.log('缓存不足')
                        }else{
                            window.peers[i].send(data)
                        }
                    }else console.log(i+" is close")
                }
            else if(window.peers[target])
                window.peers[target].send(data)
            else
                console.log("target does not exist")
        }
        this.receive=(message,sourceId)=> {
            console.log('this.receive',message)
        }
        //window.receive=this.receive

        window.addPeer=(id,channel)=>{
            scope.peers[id]=channel
